...
//            }
        ...
        reloadUserFields: function (role) {
            ...
            var type_form = $('input[name="_cred[form][type]:checked"]').val();
            credView.reloadUserFields(ag_username_is_checked, ag_nickname_is_checked, ag_password_is_checked, role, type_form);
        },
        genUserScaffold: function (refer)
        {
            if (refer == 'button') {
                var type_user_form = $('input[name="_cred[form][type]"]:checked').val();
                var cred_form_user_role = $('#cred_form_user_role').val();
                if (type_user_form == 'new' &&
                        (cred_form_user_role == 'undefined' || 
...
//            }
        ...
        onLoad: function (resp)
        {
            ...
            {
                cont2 = $('<div class="cred-accordeon-item"><a href="javascript:;" class="cred-fields-group-heading">' + cred_settings.locale.form_fields + '</a></div>');
            ...
            {
                cont2 = $('<div class="cred-accordeon-item"><a href="javascript:;" class="cred-fields-group-heading">' + cred_settings.locale.post_fields + '</a></div>');
                cont3 = $('<div class="cred-accordeon-item-inside"></div>');
                cont2.append(cont3);
                cont.append(cont2);
            ...
            {
                cont2 = $('<div class="cred-accordeon-item"><a href="javascript:;" class="cred-fields-group-heading">' + cred_settings.locale.custom_fields + '</a></div>');
                cont3 = $('<div class="cred-accordeon-item-inside"></div>');
                cont2.append(cont3);
                cont.append(cont2);
            ...
            {
                cont2 = $('<div class="cred-accordeon-item"><a href="javascript:;" class="cred-fields-group-heading">' + cred_settings.locale.taxonomy_fields + '</a></div>');
                cont3 = $('<div class="cred-accordeon-item-inside"></div>');
                cont2.append(cont3);
                cont.append(cont2);
            ...
            {
                cont2 = $('<div class="cred-accordeon-item"><a href="javascript:;" class="cred-fields-group-heading">' + cred_settings.locale.parent_fields + '</a></div>');
                cont3 = $('<div class="cred-accordeon-item-inside"></div>');
                cont2.append(cont3);
                cont.append(cont2);
                resp2 = resp.parents;
            ...
            {
                cont2 = $('<div class="cred-accordeon-item"><a href="javascript:;" class="cred-fields-group-heading">' + cred_settings.locale.extra_fields + '</a></div>');
                cont3 = $('<div class="cred-accordeon-item-inside"></div>');
                cont2.append(cont3);
                cont.append(cont2);
...
//            }
    ...
    var self = {
        ...
        doCheck: function (step)
        {
            //cred-128
            //###############################################################
            var is_form_settings = ((!step || (step && step == 2)) && $('input[name="_cred[form][type]"]').is(':visible')); //$('#cred_form_type').length;
            var type_form = $('input[name="_cred[form][type]"]:checked').val();
            ...
            var email_fields = $('.notification-sender-email');
            for (var field in email_fields) {
                if (isNaN(field))
                    break;
                if ($(email_fields[field]).val() != "" && !this.isEmail($(email_fields[field]).val())) {
                    gui.Popups.alert({
                        message: cred_settings.locale.invalid_notification_sender_email,
                        class: 'cred-dialog',
                        callback: function () {
                            $(email_fields[field]).closest('.cred_notification_settings_panel').show();
                            $(email_fields[field]).focus();
                        }
                    });
                    return false;
                }
        ...
        forms: function (useCodeMirror)
        {
            ...
            credView = new mvc.View('cred', _credModel, {
                reloadUserFields: function (ag_username_is_checked, ag_nickname_is_checked, ag_password_is_checked, role, type_form) {
                    $('#cred_form_user_role').prop("disabled", true);
                    if (typeof _busy == 'undefined' || !_busy) {
                        ...
                        $.ajax({
                            ...
                            timeout: 10000,
                            type: 'POST',
                            data: 'role=' + role + '&type_form=' + type_form + '&ag_pass=' + ag_password_is_checked + '&ag_uname=' + ag_username_is_checked + '&ag_nname=' + ag_nickname_is_checked + '&_wpnonce=' + cred_settings._cred_wpnonce,
                            dataType: 'json',
                            ...
                            complete: function () {
                                _busy = false;
                                $('#cred_form_user_role').prop("disabled", false);
                            }
                        });
                    }
                },
                init: function () {
                    ...
                    $('input[name="_cred[form][type]"]').bind('change', function () {
                        aux.check_cred_form_type();
                    });
...
//                                            timeout: 10000,
//                                            type: 'POST',
//                                            data: 'role=' + role + '&ag_uname=' + _original_cred_autogenerate_username_scaffold + '&ag_nname=' + _original_cred_autogenerate_nickname_scaffold + '&ag_pass=' + _original_cred_autogenerate_password_scaffold + '&_wpnonce=' + cred_settings._cred_wpnonce,
//                                            dataType: 'json',
//                                            success: function (resp) {
//                                                // load and dispatch event of fields loaded
//                                                aux.onLoad(resp);
...
//                                        });
                    ...
                    var _do_submit = function () {
                        ...
                        if ($('input[name="_cred[form][type]"') && $('input[name="_cred[form][type]"').length > 0) {
                            var form_form_type = $('input[name="_cred[form][type]"]:checked').val();
                            if ($.trim(form_form_type) == '')
                            {
                                gui.Popups.alert({message: cred_settings.locale.post_type_missing, class: 'cred-dialog'});
                                return false;
                            }
                        }
                        ...
                        if ($('#cred_post_status') && $('#cred_post_status').length > 0) {
                            ...
                            {
                                gui.Popups.alert({message: cred_settings.locale.post_status_missing, class: 'cred-dialog'});
                                return false;
                            }
                        }
                        ...
                        if ($('#cred_form_success_action') && $('#cred_form_success_action').length > 0) {
                            ...
                            {
                                gui.Popups.alert({message: cred_settings.locale.post_action_missing, class: 'cred-dialog'});
                                return false;
                            }
                        }
                    ...
                    /*
                     ...
                     var form_type = $('input[name="_cred[form][type]"]').val();
                     ...
                     gui.Popups.confirm({
                     'class': 'cred-dialog',
                     'message': cred_settings.locale.autogeneration_alert,
                     'buttons': [cred_settings.locale.cancel_text, cred_settings.locale.ok_text],
                     'callback': function (result) {
                     if (result == cred_settings.locale.cancel_text)
                     {
                     popup_shows = false;
                     return false;
                     }                              
                    ...
                    setTimeout(function () {
                        if (window.QTags && window.QTags.instances.hasOwnProperty(0) && window.QTags.instances[0].id == "content") {
                            Toolset.add_qt_editor_buttons(window.QTags.instances[0], CodeMirrorEditors["content"]);
                        }
                    }, 200);
            ...
            postdivrich
                    .prepend('<p class="cred-explain-text">' + cred_settings.locale.content_explain_text + '</p>');

            //Now this is added already inside the save-form-meta-box template
            //$_post
            //        .append('<input id="cred-submit" type="submit" class="button button-primary button-large" value="' + cred_settings.locale.submit_but + '" />');
